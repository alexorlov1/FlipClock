...
test('if list is rendered with string', () => {
    ...
    expect(el.querySelector('.flip-clock-card-item:first-child .top').innerHTML).toBe('a');
    expect(el.querySelector('.flip-clock-card-item:last-child .bottom').innerHTML).toBe('z');
});
